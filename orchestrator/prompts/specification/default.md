...
1. Read the alignment log at `{{ alignment_log }}`. Treat the alignment decisions and accepted assumptions as binding. **Do not expand a small task into an enterprise system.** Stay proportional to what alignment resolved and the upstream overview asks for — do not invent adjacent features, admin surfaces, multi-tenant storage, or future phases. Capture only what is genuinely required to make the requested behaviour work correctly. If the upstream feature overview or alignment log names judging, rubric, or grading criteria (e.g. "this will be evaluated on X, Y, Z"), copy them verbatim into the PRD's `Success Criteria` section. If user-facing inputs are in scope, state in `Constraints` that invalid input must produce a graceful, well-formed error — no unhandled exceptions and no 5xx — unless alignment explicitly says otherwise.
