...
- **Bootstrap is an explicit, idempotent CLI command — never implicit during a run.** `orchestrator bootstrap` is the only path that scaffolds `.cco.yaml` and marker files into a target repo; `run_pipeline` only *warns* when a repo looks unbootstrapped and offers the same primitives interactively (declined → continue without verification). Template content lives in `orchestrator/bootstrap.py`; adding a toolchain to the bootstrap roster does not require changes anywhere else in the orchestrator. See ADR-037.
