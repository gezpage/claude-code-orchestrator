...
re-invoked. See `tests/e2e_harness.py` for the shared scaffolding.
"""
...
def test_fix_cycle_resolves_architecture_changes(tmp_path):
    ...
    overrides = {
        "review:architecture:r1": h.reviewer_signal(
            "architecture",
            "changes-requested",
            findings=["Coupling between dispatcher and review_cycle"],
        ),
        "review:architecture:r2": h.reviewer_signal("architecture", "approved"),
    }
    signals = h.merge_signals(h.default_signals(out_dir, track_prompt), overrides)
    fake = h.make_fake_run_claude(signals)
    ...
    # Reviewer per-key call counts: arch hit twice (r1 + r2), impl/tests hit once.
    ...
    assert mock_claude.call_count == 13
    ...
    # (The round-1 file and Fix-Cycle-1 divider are written by the real claude
    # agent in production via the reviewer prompt — the mock does not simulate
    # those file side-effects.)
    ...
    assert "Fixed in Fix Cycle" in plan_md  # exact cycle number is a UX detail
