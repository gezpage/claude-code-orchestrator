...
### Added
- 2026-05-16: Always-on `executive_summary.md` written to the run folder root as the final post-pipeline step. The summary is generated by a new Claude finalisation stage that runs on every exit path — pass, fail, or blocked — by wrapping the stage loop and PR finalisation in `try / finally`. When PR creation produced a URL, it is forwarded into the prompt; otherwise the summary records that no PR was created. Stage failures here log warnings and never change the pipeline exit status. Issue #31. See ADR-027.
