...
### Changed
- 2026-05-14: Added a deterministic Node `package.json` manifest checker (ADR-017) that runs once per pipeline before QA/review. Detects fake/no-op quality scripts (`echo`, `exit 0`, empty), `node`/`python` script targets pointing to missing files, and likely-unused production dependencies. Blocking findings (fake scripts, missing targets) abort the QA or review stage before any LLM dispatch; advisory findings (heuristic unused-dep flagging) are written to `$RUN_FOLDER/verify/manifest-findings.{json,md}` and surfaced to QA and the implementation reviewer via the new `manifest_findings_path` variable. Section 9 of issue #69 (runtime mutation/callback probes) is deferred — the highest-leverage parts overlap with this checker, and the remaining probes need a framework that did not fit a single reviewable PR.
