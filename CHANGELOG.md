...
### Added
- 2026-05-14: E2E harness gains a `StageOutput(signal=..., artifacts=...)` wrapper so a fake stage can declare files to write before returning its signal, mirroring a real run's on-disk side-effects. `default_signals()` uses it to materialise `prd.md`, `context.md`, and `findings-track-a.md`; plain dicts continue to work for stages without artifacts.
