...
### Added
- 2026-05-14: End-to-end happy-path test (`tests/test_e2e_happy_path.py`) drives the bundled `full` profile through every stage by mocking only `_run_claude`; all prompt rendering, schema validation, signal extraction, state persistence, and plan.md rendering run for real. `ORCH_E2E_OUTPUT_DIR` env var pins run artefacts to a stable path for inspection.
