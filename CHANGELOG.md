...
### Changed
- 2026-05-17: Wave-verification helpers (`_maybe_capture_wave_baseline`, `_maybe_run_wave_verification`, `_stamp_wave_node`, `_wave_fix_then_retry`, `_append_wave_verification_section`) moved from `orchestrator/orchestrate.py` to a new `orchestrator/wave_verification.py` module. Pure relocation under issue #154 — no behaviour change. `orchestrate.py` re-imports the five names so existing call sites and `from orchestrator.orchestrate import _maybe_*` patch paths continue to resolve. The wave dispatch test stacks gain an `orchestrator.wave_verification.run_stage` patch so the `fix_then_retry` retry call stays intercepted now that the call site no longer routes through `orchestrate`'s `run_stage` binding.
