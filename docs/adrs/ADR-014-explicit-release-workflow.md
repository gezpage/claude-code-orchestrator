...
- **Gates inside the release workflow:** the workflow re-runs the full `ci.yml` quality gate (`ruff check`, `ruff format --check`, `mypy .`, `pytest tests/`, `python -m pip_audit`), then builds the wheel and sdist with `uv build`, inspects both artifacts, installs the wheel with `pip`, and runs `orchestrator --help` as a smoke test. All gates must pass before the tag is created. This duplicates work the CI run already did, but makes the release workflow self-contained: a release proves the released commit passes every gate at release time, independent of whether earlier CI runs were green.
