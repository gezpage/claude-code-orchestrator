...
- The panel JSON body is a status-derived placeholder, not the real output
  signal. ADR-004 forbids `orchestrate.py` from reading stage output files,
  and the renderer follows the same discipline; the bold "Output" link at the
  top of every panel gives users one click to the real content. Embedding
  actual JSON would require reading and parsing each stage's output file at
  every re-render and is deferred.
