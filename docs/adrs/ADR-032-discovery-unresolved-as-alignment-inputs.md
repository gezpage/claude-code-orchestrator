...
- The gate is keyed off `stage.name == "alignment"` and `stage.alignment_policy`. Adding the gate elsewhere (e.g. profile-name branching) would violate the same "config drives behaviour, not name strings" principle as ADR-030.
